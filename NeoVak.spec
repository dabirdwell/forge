...
"""
NeoVak.spec - PyInstaller specification for macOS .app bundle

This creates a self-contained NeoVak.app that:
- Opens a native window (not a browser)
- Auto-detects and starts ComfyUI
- Bundles all dependencies

Build with:
    pyinstaller NeoVak.spec
...
datas = [
    ...
    (str(project_dir / 'neovak_ui.py'), '.'),
    (str(project_dir / 'neovak_backend.py'), '.'),
    (str(project_dir / 'neovak_progress.py'), '.'),
    (str(project_dir / 'neovak_config.example.json'), '.'),
...
a = Analysis(
    [str(project_dir / 'neovak_launcher.py')],
...
app = BUNDLE(
    ...
    bundle_identifier='com.dabirdwell.neovak',
    info_plist={
        'CFBundleName': 'NeoVak',
        'CFBundleDisplayName': 'NeoVak',
